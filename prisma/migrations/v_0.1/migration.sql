...
CREATE TYPE inspection.deficiencytype_dependend AS ENUM (
    'cadet',
    'uniform'
);
ALTER TYPE inspection.deficiencytype_dependend OWNER TO "uniformServer";

CREATE TYPE inspection.deficiencytype_relation AS ENUM (
    'uniform',
    'material'
);
...
INSERT INTO authentication.assosiation 
     SELECT id, name, acronym, "useBeta"
...
INSERT INTO authentication."user" 
     SELECT id, fk_assosiation, username, name, password, role, active, "failedLoginCount" 
       FROM public."User";
--
...
INSERT INTO base.cadet
     SELECT id, fk_assosiation, firstname, lastname, active, comment, recdelete, "recdeleteUser"
        FROM public."Cadet";
--
...
INSERT INTO base.material
     SELECT id, typename, "fk_materialGroup", "actualQuantity", "targetQuantity", "sortOrder", "recdelete", "recdeleteUser"
...
INSERT INTO base.material_group
     SELECT id, fk_assosiation, description, "issuedDefault", "sortOrder", recdelete, "recdeleteUser", "multitypeAllowed"
...
INSERT INTO base.material_issued
     SELECT id, fk_material, fk_cadet, quantity, "dateIssued", "dateReturned"
...
INSERT INTO base.uniform
     SELECT id, number, "fk_uniformType", "fk_generation", fk_size, active, comment, recdelete, "recdeleteUser"
       FROM public."Uniform";
--
...
INSERT INTO base.uniform_generation
     SELECT id, "fk_uniformType", name, "fk_sizeList", "sortOrder", outdated, recdelete, "recdeleteUser"
...
INSERT INTO base.uniform_issued
     SELECT id, fk_cadet, fk_uniform, "dateIssued", "dateReturned"
...
INSERT INTO base.uniform_size 
     SELECT id, fk_assosiation, name, "sortOrder"
...
INSERT INTO base.uniform_type
     SELECT id, fk_assosiation, acronym, name, "issuedDefault", "usingGenerations", "usingSizes", "sortOrder", recdelete, "recdeleteUser", "fk_defaultSizeList"
...
CREATE TABLE inspection.deficiency_type (
    ...
    fk_assosiation character(36) NOT NULL,
    name character varying(20) NOT NULL,
    dependend inspection.deficiencytype_dependend NOT NULL,
...
INSERT INTO inspection.deficiency_type
     SELECT cdt.id,
	        cdt.fk_assosiation,
	        cdt.name,
	        CAST(CASE WHEN (cdt."addCommentToUniformitem" = TRUE) THEN 'uniform' ELSE 'cadet' END as inspection.deficiencytype_dependend),
	        CAST(CASE 
...
INSERT INTO inspection.cadet_deficiency
     SELECT d.id,
     		ci.fk_cadet,
     		CASE WHEN dt.relation = 'uniform' THEN
     			(SELECT iu.id
...
INSERT INTO inspection.cadet_inspection
     SELECT id, fk_inspection, fk_cadet, "uniformComplete", 'xx'
...
INSERT INTO inspection.deficiency
     SELECT cd.id, 
	        ...
	        'xx',
	        'xx',
	        CASE WHEN cd."dateResolved" IS NULL THEN NULL ELSE 'xx' END,
	        ci."fk_inspection",
	        (SELECT i.id
...
INSERT INTO inspection.inspection 
     SELECT id, fk_assosiation, date, null, null, active
...
ALTER TABLE ONLY base.uniform
    ADD CONSTRAINT uniform_fk_size_fkey FOREIGN KEY (fk_size) REFERENCES base.uniform_size(id) ON UPDATE RESTRICT ON DELETE SET NULL;
ALTER TABLE ONLY base.uniform
...
ALTER TABLE ONLY base.uniform_generation
    ADD CONSTRAINT uniform_generation_fk_sizelist_fkey FOREIGN KEY (fk_sizelist) REFERENCES base.uniform_sizelist(id) ON UPDATE SET NULL ON DELETE SET NULL;
ALTER TABLE ONLY base.uniform_generation
...
ALTER TABLE ONLY base.uniform_type
    ADD CONSTRAINT uniform_type_fk_default_sizelist_fkey FOREIGN KEY (fk_default_sizelist) REFERENCES base.uniform_sizelist(id) ON UPDATE SET NULL ON DELETE SET NULL;
ALTER TABLE ONLY inspection.cadet_deficiency
...
ALTER TABLE ONLY inspection.deficiency
    ADD CONSTRAINT deficiency_fk_inspection_created_fkey FOREIGN KEY (fk_inspection_created) REFERENCES inspection.inspection(id) ON UPDATE RESTRICT ON DELETE SET NULL;
ALTER TABLE ONLY inspection.deficiency
    ADD CONSTRAINT deficiency_fk_inspection_resolved_fkey FOREIGN KEY (fk_inspection_resolved) REFERENCES inspection.inspection(id) ON UPDATE RESTRICT ON DELETE SET NULL;
ALTER TABLE ONLY inspection.deficiency_type
...
CREATE OR REPLACE VIEW inspection.v_deficiency_by_cadet
 AS
 SELECT d.id,
    dt.id AS "typeId",
    dt.name AS "typeName",
...
UNION ALL
 SELECT d.id,
   ...
   FROM inspection.deficiency d
     ...
     LEFT JOIN base.uniform_issued ui ON ui.fk_uniform = u.id AND ui.date_returned IS NULL;

ALTER TABLE inspection.v_deficiency_by_cadet
    OWNER TO "uniformServer";
