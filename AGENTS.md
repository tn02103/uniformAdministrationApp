...
Every database query MUST be scoped to `fk_assosiation`. Never query without an org filter. All IDs passed from the frontend must be validated to belong to the user's organisation before use - enforced via `genericSAValidator` in every DAL function.
