...
describe('<UniformItem> create', () => {
    ...
    beforeEach(() => {
        prismaMock.uniform.findMany.mockResolvedValue([]);
        prismaMock.uniform.createMany.mockResolvedValue({ count: 4 });
    });

    afterEach(() => {
        vi.clearAllMocks();
        // Reset all mock implementations to their default state
        prismaMock.uniform.findMany.mockReset();
        prismaMock.uniform.createMany.mockReset();
    ...
    describe('successful in all allowed combinations', () => {
        ...
        it('creates with size and generation', async () => {
            ...
            expect(prismaMock.uniform.createMany.mock.calls[0][0]?.data).toHaveLength(4);
            expect(prismaMock.uniform.createMany).toHaveBeenCalledWith({
        ...
        it('creates without size, with generation', async () => {
            prismaMock.uniformType.findUniqueOrThrow.mockResolvedValue(mockTypeList[1] as any);
            prismaMock.uniform.createMany.mockResolvedValue({ count: 2 });

            await expect(create(defaultWithoutSizes)).resolves.toBe(2);

            
            expect(prismaMock.uniform.createMany.mock.calls[0][0]?.data).toHaveLength(2);
            expect(prismaMock.uniform.createMany).toHaveBeenCalledWith({
                data: expect.arrayContaining([
                    expect.objectContaining({
                        ...defaultData,
        ...
        it('creates without size, without generation', async () => {
            prismaMock.uniformType.findUniqueOrThrow.mockResolvedValue(mockTypeList[3] as any);
            prismaMock.uniform.createMany.mockResolvedValue({ count: 2 });
    ...
    describe('test type validations', () => {
        it('catches deleted or invalid types', async () => {
            prismaMock.uniformType.findUniqueOrThrow.mockRejectedValue(
            ...
            expect(prismaMock.uniform.createMany).not.toHaveBeenCalled();
        });
    });
    ...
    describe('number validations', () => {
        it('catches duplicated number with existing not deleted item of same type', async () => {
            ...
            await expect(prismaMock.uniform.findMany).toHaveBeenCalledWith({
                where: {
