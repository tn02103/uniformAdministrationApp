...
import { resignMemberDirectly } from "./directResignation";
...
describe('<returnCadetDirectly> Integration Tests', () => {
    beforeEach(async () => {
        ...
        await prisma.assosiationConfiguration.update({
            ...
            data: { anonymizationMode: AnonymizationMode.MANUAL, resignationProcessEnabled: false },
        });
    });
    ...
    it('should throw when return process is enabled', async () => {
        await prisma.assosiationConfiguration.update({
            where: { assosiationId: staticData.fk_assosiation },
            data: { resignationProcessEnabled: true },
        });

        await expect(
            resignMemberDirectly({ cadetId: ids.cadetIds[0] })
        ).rejects.toThrow("Direct cadet return is disabled when resignation process is enabled");
    });
    ...
    it('should throw if cadet is not ACTIVE', async () => {
        ...
        await expect(
            resignMemberDirectly({ cadetId })
        ).rejects.toThrow("Cadet is not ACTIVE");
    });

    it('should set cadet status to RESIGNED when anonymizationMode is MANUAL', async () => {
        const cadetId = ids.cadetIds[0]; // ACTIVE cadet

        await resignMemberDirectly({ cadetId });

        const cadet = await prisma.cadet.findUnique({ where: { id: cadetId } });
        expect(cadet!.status).toBe(CadetStatus.RESIGNED);
        expect(cadet!.deletedAt).toBeNull();
        expect(cadet!.resignedAt).not.toBeNull();
        expect(cadet!.firstname).not.toBe('XXXX');
    });

    it('should set cadet status to RESIGNED when anonymizationMode is AFTER_DAYS', async () => {
        await prisma.assosiationConfiguration.update({
            where: { assosiationId: staticData.fk_assosiation },
    ...
    it('should reject cadet from wrong org', async () => {
        ...
        await expect(
            resignMemberDirectly({ cadetId: wrongCadetId })
        ).rejects.toThrow();
    });
    ...
    it('should set dateReturned on selected uniform items', async () => {
        ...
        await resignMemberDirectly({ cadetId, selectedUniformIds });
    ...
    it('should set dateReturned on selected material items', async () => {
        ...
        await resignMemberDirectly({ cadetId, selectedMaterialIds });
