...
import { createResignationProcessSchema, CreateResignationProcessInput } from "@/zod/resignationProcess";

/**
 * Returns selected uniform items and materials for a cadet, and resolves all open deficiencies.
 * Used as part of the member resignation workflow within an existing transaction.
 */
 ...
 * @param data cadetId, resignationProcessTemplateId, and optional fields
 * @returns the created ResignationProcess
 */
export const create = (data: CreateResignationProcessInput) =>
    genericSAValidator(
        AuthRole.inspector,
        data,
        createResignationProcessSchema,
        {
            cadetId: data.cadetId,
            resignationProcessTemplateId: data.resignationProcessTemplateId,
        }
    ).then(([{ assosiation, username }, { cadetId, resignationProcessTemplateId, inspectorComment, preCheckedItemIds, finished, selectedUniformIds, selectedMaterialIds }]) =>
        prisma.$transaction(async (client) => {
            ...
            const checklistItems = await client.resignationChecklistItemTemplate.findMany({
                where: { processTemplateId: resignationProcessTemplateId, fk_assosiation: assosiation },
            });
            ...
            const resignationProcess = await client.resignationProcess.create({
                data: {
                    cadetId,
                    templateId: resignationProcessTemplateId,
            ...
            await client.cadet.update({
                where: { id: cadetId, fk_assosiation: assosiation, deletedAt: null },
                data: {
                    status: isFinished ? CadetStatus.RESIGNED : CadetStatus.RESIGNING,
                    resignedAt: now,
                },
            });

            return resignationProcess;
        })
    );
