...
import { createResignationProcess, resignMemberDirectly } from "@/dal";
...
import { ResignationProcessModalFormType, resignationProcessModalFormSchema } from "@/zod/resignationProcess";
 ...
 */
export default function MemberResignationModal(props: Props) {
 ...
 * Step 2 (if resignationProcessEnabled and templates available): Configure the exit process.
 */
function MemberResignationModalForm({
    cadetId,
    resignationProcessEnabled,
    templates,
    onClose,
    uniformLabels,
    allMaterials,
    ...
    const showProcessStep = resignationProcessEnabled && templates.length > 0;
    ...
    const sortedItems = useMemo(
        () => [...(selectedTemplate?.checklistItemTemplates ?? [])].sort((a, b) => a.sortOrder - b.sortOrder),
        [selectedTemplate]
    );
    ...
    const form = useForm<ResignationProcessModalFormType>({
        resolver: zodResolver(resignationProcessModalFormSchema),
        defaultValues: {
    ...
    async function submitProcess(data: ResignationProcessModalFormType, finished: boolean) {
        ...
        try {
            ...
            await createResignationProcess({
                cadetId,
                resignationProcessTemplateId: selectedTemplate.id,
                preCheckedItemIds,
    ...
    async function submitDirect() {
        ...
        try {
            ...
            const selectedMaterialIds = Object.entries(materialItemsValues)
                .filter(([, checked]) => checked)
                .map(([id]) => id);
            await resignMemberDirectly({ cadetId, selectedUniformIds, selectedMaterialIds });
                                    ...
                                    <>
                                        ...
                                        <div className="d-flex flex-row flex-wrap mt-2">
                                            {uniformLabels.map((label) => (
                                                <CheckboxFormField<ResignationProcessModalFormType>
                                                    key={label.id}
                                                    name={`uniformItems.${label.id}` as Path<ResignationProcessModalFormType>}
                                    ...
                                    <>
                                        ...
                                        <div className="d-flex flex-row flex-wrap mt-2">
                                            {allMaterials.map((material) => (
                                                <CheckboxFormField<ResignationProcessModalFormType>
                                                    key={material.id}
                                                    name={`materialItems.${material.id}` as Path<ResignationProcessModalFormType>}
                            ...
                            <div>
                                ...
                                {templates.length > 1 && (
                                    <div className="d-flex justify-content-center mb-3">
                                        <SelectFormField<ResignationProcessModalFormType>
                                            name="templateId"
                                ...
                                {sortedItems.map((item) => (
                                    <CheckboxFormField<ResignationProcessModalFormType>
                                        key={item.id}
                                        name={`items.${item.id}` as Path<ResignationProcessModalFormType>}
                                        label={item.label}
                                    />
                                ))}
                                <div className="mt-3">
                                    <TextareaFormField<ResignationProcessModalFormType>
                                        name="notes"
