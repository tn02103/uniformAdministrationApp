...
import MemberResignationModal from "./MemberResignationModal";
import { resignMemberDirectly } from "@/dal/cadet/resignation";
import {createResignationProcess} from "@/dal/cadet/resignation/process";
import { useCadetUniformDescriptList, useCadetMaterialMap } from "@/dataFetcher/cadet";
import { toast } from "react-toastify";
import { ResignationProcessTemplateWithItems } from "@/types/resignationProcessTypes";

// ---- Mocks ----
vi.mock("@/dal/cadet/resignation", () => ({
    resignMemberDirectly: vi.fn(),
}));
vi.mock("@/dal/cadet/resignation/process", () => ({
    createResignationProcess: vi.fn(),
}));
...
const mockTemplate: ResignationProcessTemplateWithItems = {
...
const mockTemplateAlt: ResignationProcessTemplateWithItems = {
...
function setup(props: Partial<typeof defaultProps> = {}) {
    return render(<MemberResignationModal {...defaultProps} {...props} />);
}

describe("<MemberResignationModal />", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        ...
        vi.mocked(createResignationProcess).mockResolvedValue({} as never);
    ...
    describe("step 1 — equipment confirmation", () => {
        ...
        it("shows the 'Next' button when resignationProcessEnabled with templates", () => {
            setup();
        ...
        it("shows the 'Save' button when there is no process step", () => {
            setup({ resignationProcessEnabled: false });
        ...
        it("shows the 'Save' button when resignationProcessEnabled but no templates exist", () => {
            setup({ templates: [] });
            expect(screen.getByRole("button", { name: /actions\.save/i })).toBeInTheDocument();
        });
    });
    ...
    describe("direct save (no process step)", () => {
        it("still submits direct return when process is enabled but no templates exist", async () => {
            setup({ resignationProcessEnabled: true, templates: [] });

            await userEvent.click(screen.getByRole("button", { name: /actions\.save/i }));

            expect(vi.mocked(resignMemberDirectly)).toHaveBeenCalledWith(
                expect.objectContaining({
                    cadetId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
                })
            );
            expect(vi.mocked(createResignationProcess)).not.toHaveBeenCalled();
        });
        ...
        it("calls returnCadetDirectly with only checked items when one uniform is unchecked", async () => {
            ...
            const call = vi.mocked(resignMemberDirectly).mock.calls[0][0];
        ...
        it("shows error toast when returnCadetDirectly throws", async () => {
            vi.mocked(resignMemberDirectly).mockRejectedValue(new Error("Network error"));
            setup({ resignationProcessEnabled: false });
    ...
    describe("two-step flow (with process)", () => {
        ...
        it("calls createResignationProcess with selected uniform and material IDs when starting process", async () => {
            setup();
            await userEvent.click(screen.getByRole("button", { name: /actions\.next/i }));
            ...
            expect(vi.mocked(createResignationProcess)).toHaveBeenCalledWith(
                expect.objectContaining({
                    cadetId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
                    resignationProcessTemplateId: "77777777-7777-4777-8777-777777777777",
                    finished: false,
                    selectedUniformIds: expect.arrayContaining(["11111111-1111-4111-8111-111111111111", "22222222-2222-4222-8222-222222222222"]),
                    selectedMaterialIds: expect.arrayContaining(["44444444-4444-4444-8444-444444444444", "66666666-6666-4666-8666-666666666666"]),
                })
        ...
        it("calls createResignationProcess with finished=true when saving as completed", async () => {
            setup();
            await userEvent.click(screen.getByRole("button", { name: /actions\.next/i }));
        ...
        it("does not pass unchecked uniform and material IDs to createResignationProcess", async () => {
            setup();
            ...
            const call = vi.mocked(createResignationProcess).mock.calls[0][0];
            expect(call.selectedUniformIds).not.toContain("11111111-1111-4111-8111-111111111111");
            expect(call.selectedUniformIds).toContain("22222222-2222-4222-8222-222222222222");
            expect(call.selectedMaterialIds).not.toContain("44444444-4444-4444-8444-444444444444");
            expect(call.selectedMaterialIds).toContain("66666666-6666-4666-8666-666666666666");
        ...
        it("shows error toast when createResignationProcess throws", async () => {
            vi.mocked(createResignationProcess).mockRejectedValue(new Error("server error"));
            setup();
            await userEvent.click(screen.getByRole("button", { name: /actions\.next/i }));
            await userEvent.click(screen.getAllByText(/actions\.startProcess/i)[0]);
