...
describe("RedirectTable", () => {
    it("renders the table with redirects", () => {
        ...
        expect(screen.getByText("redirects.code")).toBeInTheDocument();
        expect(screen.getByText("redirects.target")).toBeInTheDocument();
        expect(screen.getByText("redirects.active")).toBeInTheDocument();

        mockRedirects.forEach((redirect) => {
            expect(screen.getByDisplayValue(redirect.code)).toBeInTheDocument();
            expect(screen.getByDisplayValue(redirect.target)).toBeInTheDocument();
        });

        expect(container).toMatchSnapshot();
    });

    it("allows adding a new redirect row", async () => {
        ...
        const row = screen.getByRole("row", { name: /new/i });
    ...
    it('disables action-buttons when new row is open', async () => {
        ...
        const firstRow = screen.getByRole("row", { name: mockRedirects[0].code });
