...
const Footer = () => {
    ...
    return (
        <div data-testid="div_layout_footer" className="fixed-bottom bg-navy d-flex justify-content-evenly align-items-center text-white py-1">
            <Link data-testid="lnk_home" href={"/app"} className={`${pathname === "/" ? "text-primary" : ""}`}>
                <FontAwesomeIcon icon={faHome} size="xl"/>
            </Link>
            <Link data-testid="lnk_cadet" href={"/app/cadet"} className={`${pathname.includes("/app/cadet") ? "text-primary" : ""}`}>
                <FontAwesomeIcon icon={faUsers} size="xl"/>
            </Link>
            <Link data-testid="lnk_uniform" href={"/app/uniform/list"} className={`${pathname.includes("/app/uniform") ? "text-primary" : ""}`}>
                <FontAwesomeIcon icon={faShirt} size="xl"/>
            </Link>
            <Dropdown drop="up-centered">
                <DropdownToggle className="bg-transparent border-0 align-items-center" >
                    <FontAwesomeIcon icon={faGear} className={pathname.includes("/app/admin") ? "text-primary" : ""} size="xl"/>
                </DropdownToggle>
