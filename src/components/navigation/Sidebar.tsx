...
const Sidebar = ({ assosiation, username, children }: SidebarPropType) => {
  ...
  function handleLogout() {
    logout().then(() => {
      const authItemString = localStorage.getItem(
        process.env.NEXT_PUBLIC_LOCAL_AUTH_KEY as string
      );
      if (authItemString) {
      ...
      if (inspectionState?.state === "planned") {
        startInspection().then(() => {
          ...
          mutate(
            (key: any) =>
              typeof key === "string" &&
              /^(\/api\/inspection\/status)|(\/api\/cadet\/[\w\d-]+\/inspection)$/
          );
        });
      }
    }
    ...
    <div className="row p-0 m-0">
      <div className="p-0 w-auto navbar">
        ...
        <div className={`navbar-small ${showSidebar ? "show2" : ""}`}>
          <div
            data-testid="div_sidebar"
            className="d-flex flex-column align-items-start text-white min-vh-100 bg-navy text-decoration-none sticky-top z-3"
          >
            ...
            {inspectionState?.active && (
              <div
                data-testid="div_inspection"
                className="align-self-center d-inline mt-3"
              >
                {collapsed ? "" : "Inspektion: "}
                {inspectionState.inspectedCadets}/
                {inspectionState.activeCadets - inspectionState.deregistrations}
              </div>
            )}
            <ul className="flex-column mb-auto w-100 overflow-y-auto px-2 py-3">
              <NavLink
                ...
                isRoute={
                  !pathname.endsWith("cadet/new") &&
                  pathname.startsWith(`/${locale}/app/cadet`)
                }
                testId="lnk_cadet"
              />
              <NavLink
              ...
              >
                <>
                  ...
                  />
                  {(inspectionState?.active ||
                    inspectionState?.state === "unfinished" ||
            ...
            >
              {!collapsed && (
                <div className={"mx-3 my-2"}>
                  <Dropdown drop="up">
                    <Dropdown.Toggle
                      variant="primary"
                      className="border-0 text-white bg-navy p-0 fs-5"
                      data-testid={"btn_user_dropdown"}
                    >
                      {username}
                    </Dropdown.Toggle>
                    <Dropdown.Menu className="bg-navy-secondary border-white text-white">
                      ...
                      <Dropdown.Item
                        onClick={modal?.changeLanguage}
                        data-testid="btn_changeSize"
                        className="text-white bg-navy-secondary my-2 my-lg-0"
                      >
              ...
              <div
                ...
                className="btn text-white btn-lg m-2 p-0 px-2"
      ...
      <Col
        lg={collapsed ? 11 : 9}
        xl={collapsed ? 11 : 10}
        className="min-vh-100 px-0"
        onClick={() =>
          window.innerWidth < 992 && showSidebar && setShowSidebar(false)
        }
      >
